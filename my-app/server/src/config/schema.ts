...
export const CONFIG_SCHEMA: ConfigFieldDef[] = [
  ...
  // ---------- 模型参数组（llm） ----------
  {
    ...
    label: "对话模型名",
    description:
      "DeepSeek 对话模型标识。deepseek-chat 为历史名（保留兼容）；若深度思考迟迟拿不到思考过程，可切到 deepseek-flash 再试——切换后即时生效，无需改代码",
    group: "llm",
    defaultValue: "deepseek-chat",
    enabled: true,
    enum: [
      { value: "deepseek-chat", label: "deepseek-chat（当前默认）" },
  ...
  },
  {
    ...
    label: "最大 token 数",
    description:
      "单次生成的最大 token（deepseek-chat 上限 8192）。注意：开启深度思考时该值会被自动抬到 8192——思考 token 也计入输出长度，留太小会导致正文为空",
    group: "llm",
    defaultValue: 4096,
    enabled: true,
    min: 1,
  ...
  },
  {
    ...
    label: "采样温度",
    description:
      "取值越低输出越稳定（0-2）。注意：开启深度思考时该参数会被服务端忽略（不报错也不生效）",
    group: "llm",
    defaultValue: 0.3,
    enabled: true,
    min: 0,
  ...
  },
  {
    ...
    label: "思考强度",
    description:
      "深度思考的推理投入档位：low 快、high 平衡（推荐）、max 最充分但最慢且最费 token",
    group: "llm",
    defaultValue: "high",
    enabled: true,
    enum: [
      { value: "low", label: "low（快，适合交互式问答）" },
      { value: "high", label: "high（平衡，默认）" },
      { value: "max", label: "max（最充分，最慢）" },
    ],
  },
  {
    key: "llm.resume_reasoning_effort",
    type: "string",
    label: "简历优化思考强度",
    description:
      "简历优化链路的思考强度，默认 low。该链路的输出是一整份 JSON（含完整简历正文），推理与正文【共享同一个最大 token 上限】：实测 high 档推理独占约 8000 token，正文写到一半即被截断、JSON 非法。若要开更深的思考，请同步调大「最大 token 数」",
    group: "llm",
    defaultValue: "low",
    enabled: true,
    enum: [
      { value: "low", label: "low（推荐：给正文留足长度）" },
      { value: "high", label: "high（推理更长，易截断）" },
      { value: "max", label: "max（最易截断）" },
    ],
  },
  {
    key: "llm.timeout_ms",
  ...
  {
    key: "switch.deep_thinking",
    type: "boolean",
    label: "深度思考（能力总闸）",
    description:
      "关闭后任何请求都无法开启深度思考。开启时也仅当请求显式传 thinking=true 才思考——默认不思考，省时间省费用。生效范围：知识库问答、职位搜索的 Agent 决策；结构化输出节点（职位推荐理由生成、简历分析）受上游接口限制暂不支持",
    group: "switch",
    defaultValue: true,
    enabled: true,
  },
